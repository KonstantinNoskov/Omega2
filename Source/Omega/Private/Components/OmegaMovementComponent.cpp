...
void UOmegaMovementComponent::OnMovementModeChanged(EMovementMode PreviousMovementMode, uint8 PreviousCustomMode)
{
	...
	// Apply/Remove movement state tag on every movement mode update
	TMap<EMovementMode, FGameplayTag> MovementModeTagMap = FOmegaGameplayTags::Get().MovementModeToTag;
	if (MovementModeTagMap.Contains(MovementMode))			{	MovementStateTags.AddTag(MovementModeTagMap[MovementMode]);	}
	if (MovementModeTagMap.Contains(PreviousMovementMode))	{	MovementStateTags.RemoveTag(MovementModeTagMap[PreviousMovementMode]);	}
...
void UOmegaMovementComponent::PerformJump(const FInputActionValue& InputActionValue)
{
	...
	{	
		if (InputBool && IsValidJump())
		{
			...
			MovementStateTags.AddTag(FOmegaGameplayTags::Get().Movement_State_Falling);
		}
	}
}
